                ...
                <section class="row info-box mb-0">
                    <div class="col-12 p-3 m-0 info-box-header d-flex flex-wrap justify-content-between">
                        ...
                        <div class="col-12 col-md-6 text-md-right pl-1">
                            ...
                            <input name="email_activated" value="Y" type="checkbox" data-toggle="toggle" data-on="{{trans('admin.notice.on')}}" data-off="{{trans('admin.notice.off')}}" {{!empty($edit_notice_event)&&($edit_notice_event['email_activated']=='Y')?'checked':''}}>
                        </div>
                    </div>
                    <div class="d-flex flex-wrap col-12 info-item my-4">
                        <div class="form-group col-12 col-md-6">
                            <label for="email-subject">{{trans('admin.notice.emailSubject')}}</label>
                            <input id="email-subject" type="text" name="email_subject" class="form-control form-control-sm" placeholder="{{$event_name_to_chinese}}" value="{{!empty($edit_notice_event)?$edit_notice_event['email_subject']:''}}">
                        </div>
                        <div class="form-group col-12">
                            <label for="email-content">{{trans('admin.notice.description')}}</label>
                            <textarea id="email-content" class="form-control" rows="10" name="email_content">{{!empty($edit_notice_event)?$edit_notice_event['email_content']:''}}</textarea>
                        </div>
                        <div class="col-12">
